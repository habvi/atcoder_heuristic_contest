...
def _input(all_place):
    for i in range(N):
        ...
        heappush(all_place, (dst_total, dst_res_to_cus, res_x, res_y, cus_x, cus_y, i+1))
    return all_place
...
def select_50(all_place):
    ...
    heapify(place_50)
    for _ in range(M):
        dst_total, _dst, res_x, res_y, cus_x, cus_y, idx = heappop(all_place)
        heappush(place_50, (dst_total, res_x, res_y, cus_x, cus_y, idx))
    return place_50
